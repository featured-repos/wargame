...
class WarGame
  ...
  def there_is_a_tie
    cards_for_winner = [@player1_card, @player2_card]
    4.times do
      check_if_empty
      cards_for_winner += [player1_deck.cards.shift, player2_deck.cards.shift]
    end
    check_if_empty
    ...
    if @p1_tie_card == @p2_tie_card
      ...
      @p1_cards_won += [cards_for_winner, @p1_tie_card, @p2_tie_card].flatten
    elsif @p1_tie_card < @p2_tie_card
      @p2_cards_won += [cards_for_winner, @p1_tie_card, @p2_tie_card].flatten
    end
  end
